...
public class RCC_CarSelectionExample : MonoBehaviour
{
    ...
    public void SelectVehicle(int busindex = 0)
    {
        Debug.Log("BUs Index +" + busindex);
        DisableAll();
        // Registers the vehicle as player vehicle.
        _spawnedVehicles[busindex].gameObject.SetActive(true);
        ...
        //_spawnedVehicles[busindex].StartEngine();
        _spawnedVehicles[busindex].SetCanControl(true);

        // Save the selected vehicle for instantianting it on next scene.
        PlayerPrefs.SetInt("SelectedRCCVehicle", busindex);
        ...
        if (nextScene != "")
            OpenScene();
        _spawnedVehicles[busindex].semiAutomaticGear = true;
        //for (int i = 0; i < _spawnedVehicles.Count; i++)
        //{
        //    if (i != busindex)
        //    {
        //        Destroy(_spawnedVehicles[i].gameObject);
        //    }

        //}
        //vishal 21-09-2021
        GameManager.BusSimulation.SelectedVehicle(_spawnedVehicles[busindex]); // Bussimulation
    ...
    void DisableAll()
    {
        for (int i = 0; i < _spawnedVehicles.Count; i++)
        {
            if (_spawnedVehicles[i].gameObject != null)
                _spawnedVehicles[i].gameObject.SetActive(false);
        }
    }
    ...
    {
        _spawnedVehicles[selectedIndex].gameObject.SetActive(false);
