...
public class RCC_TruckTrailer : MonoBehaviour
{
    ...
    private RCC_CarControllerV3 jointtrailer;//vishal
    ...
    void Start()
    {
        ...
        {
            jointtrailer = joint.connectedBody.gameObject.GetComponent<RCC_CarControllerV3>();
    ...
    void FixedUpdate()
    {
        if (RCC_SceneManager.Instance.activePlayerCamera.TPSAutoFocus)
        {
            RCC_SceneManager.Instance.activePlayerCamera.TPSAutoFocus = false;
        }
        RCC_SceneManager.Instance.activePlayerCamera.TPSDistance = 14f;
        RCC_SceneManager.Instance.activePlayerCamera.TPSHeight = 5f;
        if (Input.GetKeyDown(RCC_Settings.Instance.trailerAttachDetach))
        {
            attached = !attached;
            if (attached)
            {
                AttachTrailer(jointtrailer);
            }
            else
            {
                DetachTrailer();
