        ...
        switch (RCC_Settings.Instance.selectedControllerType)
        {
            ...
            case RCC_Settings.ControllerType.Mobile:
                ...
                if (mobileInput)
                {
                    ...
                    inputs.throttleInput = RCC_MobileButtons.Instance.inputs.throttleInput + Mathf.Clamp01(Input.GetAxis(RCC_Settings.Instance.verticalInput));
                    inputs.brakeInput = RCC_MobileButtons.Instance.inputs.brakeInput + Mathf.Abs(Mathf.Clamp(Input.GetAxis(RCC_Settings.Instance.verticalInput), -1f, 0f));
                    inputs.steerInput = RCC_MobileButtons.Instance.inputs.steerInput + Mathf.Clamp(Input.GetAxis(RCC_Settings.Instance.horizontalInput), -1f, 1f); ;
                    inputs.handbrakeInput = RCC_MobileButtons.Instance.inputs.handbrakeInput + Mathf.Clamp01(Input.GetKey(RCC_Settings.Instance.handbrakeKB) ? 1f : 0f);
                    inputs.boostInput = RCC_MobileButtons.Instance.inputs.boostInput + Mathf.Clamp01(Input.GetKey(RCC_Settings.Instance.boostKB) ? 1f : 0f);
