...
public class RCC_Camera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (GameManager.BusSimulation.fpsRearCam.activeInHierarchy)
            GameManager.BusSimulation.fpsRearCam.SetActive(false);
        if (GameManager.BusSimulation.extraTag)
        {
            if (GameManager.BusSimulation.extraTag.FPS_cams.activeInHierarchy)
            {
                GameManager.BusSimulation.extraTag.FPS_cams.SetActive(false);
            }
        }
        ...
        switch (cameraMode)
        {
            ...
            case CameraMode.FPS:
                FPS();
                GameManager.BusSimulation.fpsRearCam.SetActive(true);
                GameManager.BusSimulation.extraTag.FPS_cams.SetActive(true);
    ...
    public void ChangeCamera()
    {
        ...
        //we need only TPS, FPS, and Wheel when game play

        if (cameraSwitchCount >= 3)// VISHAL 17/09 //
            cameraSwitchCount = 0;
    ...
    private void TPS()
    {
        ...
        if (Time.time > 1)
            currentRotation = Quaternion.Slerp(currentRotation, wantedRotation, 1f - Mathf.Exp(-TPSRotationDamping * Time.deltaTime));
