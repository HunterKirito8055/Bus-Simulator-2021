...
public class RCC_CarControllerV3 : RCC_Core
{
    ...
    public float FuelTank
    {
        ...
        {
            fuelTank = value;
            GameManager.BusSimulation.FuelAmount = Mathf.InverseLerp(0, fuelTankCapacity, value);
        }
    }
    ...
    public void KillOrStartEngine()
    {
        ...
        if (engineRunning)
        {
            GameManager.Notification.DisplayNotification("ENGINE STOPPED");
            KillEngine();
        }
        else
        {
            StartEngine();
            GameManager.Notification.DisplayNotification("ENGINE STARTED");
        }
    }
    ...
    public IEnumerator StartEngineDelayed()
    {
        ...
        if (!engineRunning)
        {
            ...
            engineRunning = true;
            fuelInput = 1f;
            RCC_MobileButtons.Instance.startButton.GetComponent<UnityEngine.UI.Image>().color = Color.green;
    ...
    public void KillEngine()
    {
        ...
        fuelInput = 0f;
        engineRunning = false;
        RCC_MobileButtons.Instance.startButton.GetComponent<UnityEngine.UI.Image>().color = Color.red;
    ...
    private void Fuel()
    {
        if (!GameManager.BusSimulation.isInArea)
        {
    ...
    {
        if (collision.gameObject.CompareTag(StringManager.PASSENGER))
        {
            GameManager.BusSimulation.GameOver(StringManager.YOUKILLEDPASSENGER);
        }
